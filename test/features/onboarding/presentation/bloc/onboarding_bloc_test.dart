...
void main() {
  ...
  test('should emit [checking, completed] when onboarding is already done', () async {
    // arrange
    when(() => mockCheckStatus()).thenAnswer((_) async => true);
    
    // assert later
    final expected = [
      predicate<OnboardingState>((state) => state.status == OnboardingStatus.checking),
      predicate<OnboardingState>((state) => state.status == OnboardingStatus.completed),
    ];
  ...
  test('should emit [checking, required] when onboarding is NOT done', () async {
    // arrange
    ...
    final expected = [
      predicate<OnboardingState>((state) => state.status == OnboardingStatus.checking),
      predicate<OnboardingState>((state) => state.status == OnboardingStatus.required),
    ];
    expectLater(bloc.stream, emitsInOrder(expected));
  ...
  test('should emit new page index when OnboardingPageChanged is added', () async {
    // arrange
    final expected = [
  ...
  test('should emit [completed] and call complete usecase when OnboardingCompletedRequested is added', () async {
    // arrange
