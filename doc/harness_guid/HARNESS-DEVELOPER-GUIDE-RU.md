...
## 6) Правила Компоновки UI (Обязательно)

- Нельзя собирать переиспользуемые виджет-деревья через `_build*` методы.
- Виджеты должны быть классами; по умолчанию используйте `StatelessWidget`.
- Виджеты должны быть максимально «тупыми»/презентационными.
- Бизнес-состояние и переходы управляются через BLoC.
- Для логически цельного feature-local компонента используйте:
  - корневой файл в `widgets/`
  - внутренние подчасти в `widgets/components/`
  - `part` в корневом файле + `part of` в подфайлах
  - приватные классы для внутренних подкомпонентов

## 7) Стандартный Workflow (End-to-End)
...
## 9) Контроль Контекста На Длинных Задачах
...
## 11) Ежедневный Короткий Чеклист
